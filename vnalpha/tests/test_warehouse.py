...
def test_ingestion_run_lifecycle(conn):
    run_id = create_ingestion_run(conn, "vnstock-service", "/v1/reference/symbols", "VN30")
    assert len(run_id) == 36  # uuid4
    rows = conn.execute("SELECT status FROM ingestion_run WHERE ingestion_run_id = ?", [run_id]).fetchall()
...
def test_upsert_symbol(conn):
    ...
    rows = conn.execute("SELECT name FROM symbol_master WHERE symbol = 'FPT'").fetchall()
...
def test_insert_raw_ohlcv(conn):
    ...
    records = [
        {"time": "2024-01-02", "interval": "1D", "open": 90.0, "high": 92.0, "low": 89.0, "close": 91.5, "volume": 1000000.0},
    ]
    inserted = insert_raw_ohlcv(conn, run_id, "FPT", records, "kbs", "pass", "2024-01-02T09:00:00")
    assert inserted == 1
