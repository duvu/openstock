...
def test_execute_prepared_turn_records_approval_and_returns_validated_artifact_only_answer(
    ...
    approval_row = conn.execute(
        "SELECT plan_digest, code_digest, input_references_json, correlation_id, approver "
        "FROM sandbox_approval"
    ).fetchone()
    assert approval_row == (
        prepared.plan_hash,
        step.arguments["code_digest"],
        "[]",
        step.arguments["correlation_id"],
        "user",
    )
    ...
    serialized_answer = json.dumps(answer.to_dict(), ensure_ascii=False)
    assert _STDOUT_SECRET not in serialized_answer
...
def test_execute_prepared_turn_emits_guard_rejection_without_starting_runner(
    ...
    def unsafe_program(_purpose: str) -> _GeneratedProgram:
        return _GeneratedProgram(
            code="import requests\n",
            summary="Imports requests and would violate the sandbox network policy.",
        )
    ...
    service = _service(
        ...
        code_generator=unsafe_program,
    )

    plan = service.materialize_assistant_plan(_sandbox_plan("unsafe purpose"))
    prepared = _prepared_turn(plan)
    service.approve_prepared_turn(prepared)
    answer = service.execute_prepared_turn(prepared)
...
def test_execute_prepared_turn_marks_runtime_failure_and_does_not_synthesize_streams(
    tmp_path: Path,
) -> None:
    conn = duckdb.connect(":memory:")
    run_migrations(conn=conn)
