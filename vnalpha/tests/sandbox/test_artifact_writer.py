...
class _ManifestEntryPayload(BaseModel):
    model_config: ClassVar[ConfigDict] = ConfigDict(
        frozen=True, extra="forbid", strict=True
    )
...
class _ManifestPayload(BaseModel):
    ...
    entries: tuple[_ManifestEntryPayload, ...]
...
def test_persist_outputs_preserves_bytes_and_writes_deterministic_manifest(
    tmp_path: Path,
) -> None:
    from vnalpha.sandbox.artifact_writer import SandboxArtifactWriter

    result_json = b'{"unvalidated":true,"value":"\xff"}\n'
    summary_markdown = "# Summary\n\nUnicode: Việt Nam\n"
    with _storage(tmp_path) as storage:
        writer = SandboxArtifactWriter(storage)
        writer.persist_request(_job())
        manifest = writer.persist_outputs(result_json, summary_markdown)

        manifest_bytes = (storage.job_dir / "manifest.json").read_bytes()
        manifest_json = _ManifestPayload.model_validate_json(manifest_bytes)
        assert (storage.job_dir / "output/result.json").read_bytes() == result_json
        assert (
            storage.job_dir / "output/summary.md"
        ).read_bytes() == summary_markdown.encode()
        assert manifest_bytes == manifest.to_json_bytes()
        assert (
            manifest_bytes
            == writer.persist_outputs(result_json, summary_markdown).to_json_bytes()
        )
        assert manifest_bytes.endswith(b"\n")
        assert [entry.path for entry in manifest_json.entries] == [
            "generated_code.py",
        ...
        expected_entries = (
            ("generated_code.py", _job().code.encode(), "text/x-python"),
            (
                "inputs/references.json",
                b'{\n  "approved_read_paths": [\n    "inputs/reference.csv"\n  ]\n}\n',
                "application/json",
            ),
            ("output/result.json", result_json, "application/json"),
            ("output/summary.md", summary_markdown.encode(), "text/markdown"),
            (
                "request.json",
                (storage.job_dir / "request.json").read_bytes(),
                "application/json",
        ...
        for entry, (path, content, media_type) in zip(
            manifest_json.entries, expected_entries, strict=True
        ):
            assert entry.path == path
            assert entry.byte_length == len(content)
            assert entry.media_type == media_type
            assert entry.sha256 == hashlib.sha256(content).hexdigest()
        assert all(entry.path != "manifest.json" for entry in manifest_json.entries)
...
def test_closed_storage_writes_no_manifest(tmp_path: Path) -> None:
    from vnalpha.sandbox.artifact_writer import SandboxArtifactWriter

    storage = _storage(tmp_path)
    writer = SandboxArtifactWriter(storage)
    _ = writer.persist_request(_job())
    storage.close()
    with pytest.raises(SandboxArtifactPathError):
        _ = writer.persist_outputs(b"{}", "# Summary\n")

    assert not (storage.job_dir / "manifest.json").exists()


def test_output_retry_removes_previous_manifest_when_summary_replacement_fails(
    ...
    with _storage(tmp_path) as storage:
        writer = SandboxArtifactWriter(storage)
        writer.persist_request(_job())
        _ = writer.persist_outputs(b'{"version":1}', "# First\n")
        manifest_path = storage.job_dir / "manifest.json"
        previous_manifest = manifest_path.read_bytes()
        original_replace = os.replace

        def fail_summary_replacement(
            source: str,
            destination: str,
            *,
            ...
            if destination == "summary.md":
                raise OSError("summary replacement failed")
            original_replace(
                source,
                destination,
        ...
        monkeypatch.setattr(os, "replace", fail_summary_replacement)

        with pytest.raises(SandboxArtifactPathError):
            _ = writer.persist_outputs(b'{"version":2}', "# Second\n")

        assert (storage.job_dir / "output/result.json").read_bytes() == b'{"version":2}'
        assert not manifest_path.exists()
        assert previous_manifest != b""
