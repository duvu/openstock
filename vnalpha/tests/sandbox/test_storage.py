...
def test_bounded_read_returns_exact_regular_file_bytes(tmp_path: Path) -> None:
    from vnalpha.sandbox.storage import SandboxArtifactStorage
    ...
    with SandboxArtifactStorage(run_context, SandboxJobId("job-001")) as storage:
        _ = storage.write_atomic_bytes("output/result.json", b'{"result":"exact"}')

        # When
        content = storage.read_bounded_regular_file("output/result.json", max_bytes=64)

        # Then
        assert content == b'{"result":"exact"}'


def test_bounded_read_reports_missing_artifact(tmp_path: Path) -> None:
    ...
    with SandboxArtifactStorage(run_context, SandboxJobId("job-001")) as storage:
        # When / Then
        with pytest.raises(SandboxArtifactNotFoundError):
            _ = storage.read_bounded_regular_file("output/missing.json", max_bytes=64)


def test_bounded_read_rejects_symlinked_parent(tmp_path: Path) -> None:
    from vnalpha.sandbox.storage import SandboxArtifactPathError, SandboxArtifactStorage
    ...
    outside_dir = tmp_path / "outside"
    outside_dir.mkdir()
    _ = (outside_dir / "result.json").write_bytes(b"outside")
    with SandboxArtifactStorage(run_context, SandboxJobId("job-001")) as storage:
        (storage.job_dir / "output").symlink_to(outside_dir, target_is_directory=True)

        # When / Then
        with pytest.raises(SandboxArtifactPathError):
            _ = storage.read_bounded_regular_file("output/result.json", max_bytes=64)


def test_bounded_read_rejects_symlinked_leaf(tmp_path: Path) -> None:
    from vnalpha.sandbox.storage import (
        SandboxArtifactStorage,
    ...
    _ = outside_path.write_bytes(b"outside")
    with SandboxArtifactStorage(run_context, SandboxJobId("job-001")) as storage:
        output_dir = storage.ensure_directory("output")
        (output_dir / "result.json").symlink_to(outside_path)

        # When / Then
        with pytest.raises(SandboxArtifactTypeError):
            _ = storage.read_bounded_regular_file("output/result.json", max_bytes=64)


def test_bounded_read_rejects_directory(tmp_path: Path) -> None:
    from vnalpha.sandbox.storage import (
        SandboxArtifactStorage,
        SandboxArtifactTypeError,
    )
    ...
    with SandboxArtifactStorage(run_context, SandboxJobId("job-001")) as storage:
        _ = storage.ensure_directory("output/directory")

        # When / Then
        with pytest.raises(SandboxArtifactTypeError):
            _ = storage.read_bounded_regular_file("output/directory", max_bytes=64)


def test_bounded_read_rejects_fifo_without_blocking(tmp_path: Path) -> None:
    from vnalpha.sandbox.storage import (
        SandboxArtifactStorage,
        SandboxArtifactTypeError,
    )
    ...
    with SandboxArtifactStorage(run_context, SandboxJobId("job-001")) as storage:
        output_dir = storage.ensure_directory("output")
        os.mkfifo(output_dir / "stream")

        # When / Then
        with pytest.raises(SandboxArtifactTypeError):
            _ = storage.read_bounded_regular_file("output/stream", max_bytes=64)


def test_bounded_read_rejects_fstat_size_over_limit(tmp_path: Path) -> None:
    from vnalpha.sandbox.storage import SandboxArtifactSizeError, SandboxArtifactStorage
    ...
    with SandboxArtifactStorage(run_context, SandboxJobId("job-001")) as storage:
        _ = storage.write_atomic_bytes("output/result.json", b"too large")

        # When / Then
        with pytest.raises(SandboxArtifactSizeError):
            _ = storage.read_bounded_regular_file("output/result.json", max_bytes=2)


def test_bounded_read_rejects_growth_beyond_limit(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    from vnalpha.sandbox.storage import SandboxArtifactSizeError, SandboxArtifactStorage
    ...
    with SandboxArtifactStorage(run_context, SandboxJobId("job-001")) as storage:
        _ = storage.write_atomic_bytes("output/result.json", b"abc")

        def report_stale_size(fd: int) -> os.stat_result:
...
@pytest.mark.parametrize("raw_path", ("/tmp/escaped", "../escaped", "output\\escaped"))
def test_bounded_read_rejects_unsafe_path(tmp_path: Path, raw_path: str) -> None:
    from vnalpha.sandbox.storage import SandboxArtifactPathError, SandboxArtifactStorage
    ...
    with SandboxArtifactStorage(run_context, SandboxJobId("job-001")) as storage:
        # When / Then
        with pytest.raises(SandboxArtifactPathError):
            _ = storage.read_bounded_regular_file(raw_path, max_bytes=64)


def test_bounded_read_rejects_closed_storage(tmp_path: Path) -> None:
    from vnalpha.sandbox.storage import SandboxArtifactPathError, SandboxArtifactStorage
    ...
    storage = SandboxArtifactStorage(run_context, SandboxJobId("job-001"))
    _ = storage.write_atomic_bytes("output/result.json", b"contained")
    storage.close()
