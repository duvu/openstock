...
MISSING_DATA_TEMPLATES = {
    ...
    "no_canonical_ohlcv": (
        "No canonical OHLCV found for {symbol}. "
        "Run `vnalpha build canonical` first."
    ),
    "no_watchlist": (
...
9. Shortlist and scenario outputs are research-prioritization artifacts requiring human review.
...
class AnswerSynthesizer:
    ...
    def synthesize(
        ...
        if research_intent and not answer.grounded_source_refs:
            answer.grounded_source_refs = available_source_refs(plan, tool_outputs)
    ...
    def _deterministic_fallback(
        ...
        *,
        ...
        if not groundedness.passed or not policy.passed:
            detail = "; ".join(
                [*groundedness.messages, *policy.violations]
...
def _validate_context_answer(
    ...
    if _requires_caveat_first(tool_outputs) and not answer.summary.lower().startswith(
        ...
        raise SynthesisError(
            "Context synthesis must be caveat-first for limited data."
        )
