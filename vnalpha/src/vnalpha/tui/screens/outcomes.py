    ...
    class OutcomeScreen(Screen):
        ...
        def _build_summary(self) -> str:
            try:
                ...
                conn.close()
                if result is None:
                    return f"No outcome data for {self.target_date} horizon={self.horizon}"
                hit_rate_str = f"{result['hit_rate']:.1%}" if result.get("hit_rate") is not None else "—"
                failure_rate_str = f"{result['failure_rate']:.1%}" if result.get("failure_rate") is not None else "—"
                return (
        ...
        def _populate_candidates_table(self) -> None:
            try:
                ...
                for row in rows:
                    ...
                    table.add_row(
                        row["symbol"], row["outcome_status"], score, fwd, exc,
        ...
        def _populate_buckets_table(self) -> None:
            try:
                ...
                for row in rows:
                    ...
                    failure_rate_str = f"{row['failure_rate']:.1%}" if row["failure_rate"] is not None else "—"
                    table.add_row(
                        row["score_bucket"], str(row["candidate_count"] or 0), fwd,
                        hit_rate_str, failure_rate_str,
                    )
            except Exception as exc:
        ...
        def _populate_setups_table(self) -> None:
            try:
                ...
                for row in rows:
                    ...
                    failure_rate_str = f"{row['failure_rate']:.1%}" if row["failure_rate"] is not None else "—"
                    table.add_row(
                        row["setup_type"], str(row["candidate_count"] or 0), fwd,
                        hit_rate_str, failure_rate_str,
                    )
            except Exception as exc:
        ...
        def _populate_risks_table(self) -> None:
            try:
                ...
                for row in rows:
                    ...
                    failure_rate_str = f"{row['failure_rate']:.1%}" if row["failure_rate"] is not None else "—"
                    table.add_row(
                        row["risk_flag"], str(row["candidate_count"] or 0), fwd,
                        hit_rate_str, failure_rate_str,
                    )
            except Exception as exc:
