...
The ratio is measured against externally observable behavior, not lines of code or private implementation helpers.

- Private helpers and internal branches do not receive separate tests when their behavior is already exercised by the owning public feature/function.
- A single authoritative test may use table-driven assertions to cover equivalent inputs and important edge conditions without expanding them into many separately collected test nodes.
- Repository, service, CLI, TUI and assistant layers must not repeat the same domain behavior tests. The application/public contract owns the authoritative test; surfaces keep only bounded adapter/parity checks where they expose a distinct public function.
- A high-impact boundary such as point-in-time exclusion, transaction rollback, crash recovery, writer exclusion, policy approval or package state preservation is treated as its own public risk contract and may own one test.
- Adding a new authoritative test requires a new public feature/function or replacement of an obsolete test.
