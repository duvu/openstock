...
- [x] 19. Add red/green legacy post-approval, real safe-plan approval dispatch, truncation-safe Rich escaping, protocol-valid quoted/short/crop-boundary credential redaction across public, database-summary and every file-backed error field, benign prose/endpoint compatibility, metadata-only persistence and assistant error-contract regressions; correct every reproduced defect and isolate affected tests from ambient localhost/default-warehouse state.
