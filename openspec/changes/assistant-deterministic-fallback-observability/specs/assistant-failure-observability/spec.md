...
When a request reaches a downstream assistant lifecycle failure, CLI and TUI SHALL
expose the same sanitized diagnostic fields: a finite stage, stable category,
correlation ID, runtime build SHA, model route when selected, and trace ID when it
exists. They SHALL NOT expose credentials, raw provider payloads, prompts, or raw
exception text.
