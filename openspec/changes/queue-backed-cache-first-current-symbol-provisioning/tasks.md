...
- [x] 6.3 Add priority escalation and explicit shared-job cancellation warnings. Evidence: `8fa2f313cdb1a7674fd6b6bb074dfbef153417bd`; the current-symbol contract proves active requests share one priority-escalated job, unconfirmed `jobs cancel` preserves it with an explicit shared-job warning, confirmed cancellation is administrative, and `jobs retry` creates a new typed job.
