...
- [x] 0.8 Keep TODO edits routed through ComposerInput commands.
...
- [x] 3.7 Render recently done items if available.
...
- [x] 6.6 Persist manual preference if existing config/workspace context supports it. N/A: no existing config or workspace display-preference field exists; schema expansion is out of scope.
- [x] 6.7 Add toggle tests.

## 7. TODO commands

- [x] 7.1 Decide whether `/todo` commands are implemented in this PR or mapped to `/context task`.
- Native `/todo` commands persist immutable `WorkspaceState.open_tasks` mutations; the panel remains intentionally read-only.
- [x] 7.2 Add `/todo list` if implemented.
- [x] 7.3 Add `/todo add <text>` if implemented.
- [x] 7.4 Add `/todo done <id>` if implemented.
- [x] 7.5 Add `/todo block <id>` if implemented.
- [x] 7.6 Add `/todo clear-done` if implemented.
- [x] 7.7 Refresh TodoPanel after TODO-changing commands.
- [x] 7.8 Add command tests if commands are implemented.
...
- [x] 8.3 Include system-generated follow-ups where appropriate.
...
- [x] 11.5 Emit TODO item add/update events if commands are implemented.
...
- Validation evidence (2026-07-10): isolated-warehouse `make test-vnalpha` exited 0; `make lint-vnalpha` passed with 317 files format-clean; `make verify-r4` passed; `openstock-verify --ci` passed with 16 OK, 1 existing systemd warning, and 0 failures.
