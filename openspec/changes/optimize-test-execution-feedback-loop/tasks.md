...
- [x] 4.4 Update `openstock-ci.yml` so consistency always runs and smoke/domain/full jobs consume classifier outputs independently.
- [x] 4.5 Update package workflow routing so packaging/release-relevant changes run full Debian acceptance and docs-only changes do not.
- [x] 4.6 Make the Required merge gate accept only `success` or deliberate `skipped` conclusions and fail every other/missing conclusion.
- [x] 4.7 Add workflow/consistency regressions proving docs-only safe skips, unknown-path failure, parallel job eligibility and fail-closed Required conclusions.
...
- [x] 5.2 Consolidate repeated schema table counts/lists into one canonical schema manifest while retaining fresh migration, idempotency, upgrade and rollback contracts.
