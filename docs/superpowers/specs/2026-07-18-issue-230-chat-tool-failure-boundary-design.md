...
`user:password`, including credentials cut at the bounded scan edge. A JWT
prefix is redacted conservatively when the bounded scan ends before its first
segment delimiter, and unsecured JWTs with empty signatures are covered. Bearer
...
userinfo are redacted. Common prose wrappers around a grammatical endpoint are
preserved; square brackets remain escaped only where required for Rich-neutral
public rendering.
...
Credential-bearing key names are sanitized as well as their values. Prefix
matching requires a credential-bearing affix, preserving ambiguous operational
metrics and statuses such as `token_budgets`, token estimates and the provider's
safe `auth_status` structure.
